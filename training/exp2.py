...
if __name__ == "__main__":
	...
	output_path = f"../results/exp2"
	data_path = "../data"
	if not os.path.exists(output_path):
		os.mkdir(output_path)
		os.mkdir(f"{output_path}/checkpoint")

	best_val_acc_list = [[] for i in range(11)]
	best_val_acc_list[0] = np.zeros(10)					# dummy value
	...
	for sub_id in range(1, 11):
		for fold in range(10):
			...
			np.save(f"{output_path}/{sub_id}_{fold}_config.npy", config)
			...
			for epoch in range(config['epochs']):
				...
				if accuracy > max_acc:
					...
					}, f"{output_path}/checkpoint/{sub_id}_{fold}.pt")

				duration = time.time() - start_time
				with open(f"{output_path}/{sub_id}_{fold}_output.txt", 'a') as file:
					for log in logs:
						file.write(f"{log}\n")
				...
				with open(f"{output_path}/{sub_id}_{fold}_result.txt", 'a') as file:
					file.write(f"epoch {epoch}: validation acc: {accuracy} | macrof1: {macro_score} | microf1: {micro_score} | loss: {val_running_loss / val_total} | duration: {duration}\n")

			best_val_acc_list[sub_id].append(max_acc)
			...
			# plt.show()
			plt.savefig(f'{output_path}/{sub_id}_{fold}_training.png', dpi=300)
