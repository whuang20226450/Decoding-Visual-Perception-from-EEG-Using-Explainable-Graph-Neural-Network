...
def _plot_sensors(pos, info, picks, colors, bads, ch_names, title, show_names,
    ...
    if kind == '3d':
        ...
        new_pos = np.array([(x, y) for i,(x,y) in enumerate(pos) if i not in [124,125,126,127]])

        # move the 48, 118 inside a little bit
        ref_dist, scale_factor = abs(pos[47,0] - pos[118,0]), 0.08
        new_pos[47,0] += ref_dist * scale_factor
        new_pos[118,0] -= ref_dist * scale_factor

        # move all nodes inside a little bit
        for i in range(len(new_pos)):
            new_pos[i, 0] *= 0.9
            new_pos[i, 1] *= 0.95

        pts = ax.scatter(new_pos[:, 0], new_pos[:, 1], picker=True, clip_on=False,
